...
def get_val_info(model, valloader, loss_fn, device, use_tqdm=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            x, preds_three_channels, preds_one_channel = model(allimgs.to(device), rots.to(device),
            ...
            # loss
            total_loss += loss_fn(preds_one_channel, binimgs).item() * preds_one_channel.shape[0]

            # iou
            intersect, union, _ = get_batch_iou(preds_one_channel, binimgs)
