...
def eval_loop(preprocess_fn, network_factory, data_x, data_y, camera_indices,
    ...
    def mAP_metric():
        mask = np.ones((probe_x_var.shape[0], gallery_x_var.shape[0]))
        return metrics.streaming_mean_averge_precision(
            probe_x_var, probe_y_var, gallery_x_var, gallery_y_var,
            good_mask=mask, measure=distance_measure)

    # names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
    #     "Precision@%d" % k: cmc_metric_at_k(k) for k in [1, 5, 10, 20]})
    
    names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
        "Precision_1": cmc_metric_at_k(1),
        "Precision_5": cmc_metric_at_k(5),
        "Precision_10": cmc_metric_at_k(10),
        "Precision_20": cmc_metric_at_k(20),
        'mAP': mAP_metric()})
